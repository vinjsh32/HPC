...
TEST_F(AdvancedMathTest, AESSubstitutionBox) {
    ...
    // AES S-box should have exactly 256 valid input-output mappings
    EXPECT_GT(obdd_count_nodes(bdd), 256); // Should be complex
...
TEST_F(AdvancedMathTest, DESSubstitutionBox) {
    ...
    // DES S-box should have 64 valid mappings (6 input bits → 4 output bits)
    EXPECT_GT(obdd_count_nodes(bdd), 64);
